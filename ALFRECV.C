...
void AlfRecvMst(void)
{
	...
	ckputss(0, 4, "   部門   品種  ", False, CLR_BASE);
	ckputss(0, 6, "  [    ] [    ] ", False, CLR_BASE);
	ckputss(0, 8, "   担当   ﾚｼｰﾄ", False, CLR_BASE);
	ckputss(0, 10, "  [    ] [    ] ", False, CLR_BASE);
	ckputss(0, 12, "    商品       ", False, CLR_BASE);
	ckputss(0, 14, "  [      ]    ", False, CLR_BASE);
	ckputss(0, 16, "     CLR:中止   ", False, CLR_BASE);
	...
	ckputss(0, 2, "     受信中     ", False, CLR_BASE);
	for (;;)
	{
		...
		// #endif
		ckprintf(5, 6, False, CLR_BASE, "%2ld", BUMSTCnt);
	}
	...
	for (;;)
	{
		...
		// #endif
		ckprintf(10, 6, False, CLR_BASE, "%2ld", HIMSTCnt);
	}
	...
	for (;;)
	{
		...
		// #endif
		ckprintf(12, 10, False, CLR_BASE, "%2ld", REMSTCnt);
	}
	...
	/* レシートマスタ受信準備 */
	/***端末を特定できる情報を２０バイト以内でセット***/
	memcpy(_PrefixRec.TermNo, "商品マスタ受信", 14);
	/***ファイル名***/
	memcpy(_PrefixRec.FileName, "FILE5", 5);
	_AlfInf.recvLen = sizeof(TSMST) + 2;
	// #if DEBUG
	// 	displayMsg(_AlfInf.recvLen);
	// #endif
	sprintf(dspBuf, "%0*d", sizeof(_PrefixRec.BlockLen), _AlfInf.recvLen);
	...
	for (;;)
	{
		...
		if (ret == COM_EOF)
			...
			goto EndRecv;
		if (TSMSTCnt >= TSMST_MAX)
			continue;
		memcpy(&tsmst, _RecvBuf, sizeof(tsmst));
		ram_write(TSMSTCnt, &tsmst, TSMSTF);
		++TSMSTCnt;
		// #if DEBUG
		// 	msgbox( LCD_MSGBOX_WARNING, 0x00, "", &remst, "確認", 0x00 );
		// #endif
		ckprintf(3, 14, False, CLR_BASE, "%5ld", TSMSTCnt);
	}
...
EndRecv:
	...
	{
		if (ret == COM_ABORT)
			ckputss(0, 2, "    強制終了    ", False, CLR_BASE);
		else if (ret == COM_TIMEOUT)
			ckputss(0, 2, "  タイムアウト  ", False, CLR_BASE);
		else if (ret == COM_ERROR)
			ckputss(0, 2, "   通信エラー   ", False, CLR_BASE);
		else
			ckputss(0, 2, "    異常終了    ", False, CLR_BASE);
		beep(50, 1);
	...
	{
		if (BUMSTCnt)
			ctrl.BUMSTCnt = BUMSTCnt;
		if (HIMSTCnt)
			ctrl.HIMSTCnt = HIMSTCnt;
		if (TAMSTCnt)
			ctrl.TAMSTCnt = TAMSTCnt;
		if (REMSTCnt)
			ctrl.REMSTCnt = REMSTCnt;
		if (TSMSTCnt)
			ctrl.TSMSTCnt = TSMSTCnt;
		ram_write(0, &ctrl, CTRLF);
		ckputss(0, 2, "    受信完了    ", False, CLR_BASE);
		// beeb( 50, 2, 20 );
		beep(100, 1);
		ckputss(0, 16, "    ENT:確認    ", False, CLR_BASE);
	}
	while (!okng())
		;
	offkey(1);
