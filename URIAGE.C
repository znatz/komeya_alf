	...
	switch( item ){
		...
		case CODE1:
			...
			if( SaveItem != CODE1){
				...
				if( memcmp( info.tanto,"00",2 ) == 0 ){
					ckputss( 0,  0, "<練習ﾓｰﾄﾞ>      ", False, CLR_PR_TITLE );
					ckprintf(10, 0, False, CLR_PR_TITLE , "%4d件", ctrl.InfoUrCnt );
				}else{
					ckputss( 0,  0, "<売上入力>      ", False, CLR_UR_TITLE );
		...
		case GENKIN:
			if( SaveItem != GENKIN){
				...
				ckputss( 0,  8, " ｸﾚｼﾞｯﾄ :       ", False, CLR_BASE );//#02
		...
		case CREDIT:
			if( SaveItem != CREDIT){
				...
				ckputss( 0,  8, " ｸﾚｼﾞｯﾄ ", False, CLR_SI_TITLE );
		...
		// 		insComma( atoln( Genkin, sizeof( Genkin )), strGoukei );
		// 		ckprintf(9,  6, False, CLR_BASE, "%7s", strGoukei );
		// 		ckputss( 0,  8, " ｸﾚｼﾞｯﾄ :       ", False, CLR_BASE );
		// 		insComma( atoln( Credit, sizeof( Credit )), strGoukei );
