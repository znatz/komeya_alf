		...
		if( ret == ENTRY || ret == SENTRY || ret == SKIP || ret == BARENT ){
			...
			if( item == SHOP ){
				...
				if( atoin( tenpo.tenpo2, 2 ) >= 1 ){
					int intChk1 = strcmp( tenpo.tenpo1[0],tenpo.tenpo2[0]);
					int intChk2 = strcmp( tenpo.tenpo1[1],tenpo.tenpo2[1]);
			...
			/*else if( item == TANTO ){
				...
				if( atoin( idata.Code1, 3 ) > 500 && atoin( idata.Code1, 3 ) < 999 ){
					item = CODE2;
					continue;
				} else if( !memcmp( idata.Code1, "45", 2 ) || !memcmp( idata.Code1, "49", 2 ) || !memcmp( idata.Code1, "20", 2 ) ){
				...
				if( atoin( idata.Code1, 3 ) > 500 && atoin( idata.Code1, 3 ) < 999 ){
					if( (idata.Code2[0] == '0' || idata.Code2[0] == '1' || idata.Code2[0] == '2' || idata.Code2[0] == '3') && idata.Code2[12] != ' ' ){
						if(Func == 1){
							item = DENTRY;
						}
						if(Func == 2){
						...
						/* 上記に該当しない場合はエラー */
						memset( idata.Code2, 0x00, sizeof( tdata.Code2 ) );
						beep( 10, 2 );
						continue;
					}
				} else if( !memcmp( idata.Code1, "2289", 4 ) || !memcmp( idata.Code1, "2299", 4 ) ){
