	...
	switch( item ){
		...
		case CODE1:
			if( SaveItem != CODE1)
			{
				...
				ckputss( 0, 14, "F1:戻る F2:複数 ", False, CLR_BASE );
				ckputss( 0, 16, "        F4:削除 ", False, CLR_BASE );
...
void Nscan( void )
{
	...
	while( item ){
		...
		switch( item ){
			...
			case NESAGE1:
				//memset( ndata.Code3, 0x00, sizeof( ndata.Code3 ) );
				ret = CodeInput(  4, 10, ndata.Code3, sizeof( ndata.Code3 ), 
								 BCR_NOTDRW | BCR_WPC | KEY_FUNC );
				break;
			case NESAGE2:
				//memset( ndata.Code4, 0x00, sizeof( ndata.Code4 ) );
				ret = CodeInput(  4, 12, ndata.Code4, sizeof( ndata.Code4 ), 
								 BCR_NOTDRW | BCR_WPC | KEY_FUNC );
				break;
			case DENTRY:
				/* 登録 */
		...
		if( ret == ENTRY || ret == SENTRY || ret == SKIP || ret == BARENT ){
			/*if( item == SHOP ){
				...
				if( !memcmp( ndata.Code4, "30", 2 )){//&& strlen( ndata.Code4 ) == 8 ){
					memset( ndata.Code3, 0x00, sizeof( ndata.Code3 ) );
			...
			if( item == CODE1 ){
				...
				if( Func == 1 ){
					Func = 2;
					ckputss( 0, 8, "数量:           ", False, CLR_BASE );
					ckputss( 0, 14, "F1:ﾒﾆｭｰ F2:1点  ", False, CLR_BASE );
				}
				else if( Func == 2){
					Func = 1;
					ckputss( 0, 8, "                ", False, CLR_BASE );
					ckputss( 0, 14, "F1:ﾒﾆｭｰ F2:複数 ", False, CLR_BASE );
				}
			}
			else if( item == NESAGE2 ){
				/* 登録 */
