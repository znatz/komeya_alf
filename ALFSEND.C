...
static short TDataSend(void)
{
	...
	for (i = 0; i < ctrl.TDataCnt; i++)
	{
		...
		/* 日付(YYYY/MM/DD) */
		memcpy(&_SendBuf[sPos], tdata.Date, 4);
		sPos += 4;
		_SendBuf[sPos] = '/';
		++sPos;
		memcpy(&_SendBuf[sPos], &tdata.Date[4], 2);
		sPos += 2;
		_SendBuf[sPos] = '/';
		++sPos;
		memcpy(&_SendBuf[sPos], &tdata.Date[6], 2);
		sPos += 2;
		/* 区切り文字 */
		_SendBuf[sPos] = ',';
		++sPos;
		/* 店舗コード */
		memcpy(&_SendBuf[sPos], tdata.Shop, sizeof(tdata.Shop));
		sPos += sizeof(tdata.Shop);
		/* 区切り文字 */
		_SendBuf[sPos] = ',';
		++sPos;
		/* 店舗2コード */
		...
		++sPos;
		/* 上段バーコード */
		memcpy(&_SendBuf[sPos], tdata.Code1, sizeof(tdata.Code1));
		sPos += sizeof(tdata.Code1);
		/* 区切り文字 */
		_SendBuf[sPos] = ',';
		++sPos;
		/* 下段バーコード */
		...
		++sPos;
		/* DENSOに合わせるため */
		memcpy(&_SendBuf[sPos], "00000000,", 9);
		sPos += 9;
		/* 数量 */
		memcpy(&_SendBuf[sPos], "00000", 5);
		sPos += 5;
		memcpy(&_SendBuf[sPos], tdata.Num, sizeof(tdata.Num));
		sPos += sizeof(tdata.Num);
		_SendBuf[sPos] = ',';
		++sPos;
		/* 伝票番号 */
		memset(&_SendBuf[sPos], '0', 8);
		sPos += 8;
		/* DENSOに合わせるため */
		memcpy(&_SendBuf[sPos], ",0", 2);
		sPos += 2;
		/* 改行コード */
		_SendBuf[sPos] = 0x0d;
		++sPos;
...
static short SDataSend(void)
{
	...
	for (i = 0; i < ctrl.SDataCnt; i++)
	{
		...
		memset(_SendBuf, ' ', sizeof(_SendBuf));
		/* 日付(YYYY/MM/DD) */
		memcpy(&_SendBuf[sPos], sdata.Date, 4);
		sPos += 4;
		_SendBuf[sPos] = '/';
		++sPos;
		memcpy(&_SendBuf[sPos], &sdata.Date[4], 2);
		sPos += 2;
		_SendBuf[sPos] = '/';
		++sPos;
		memcpy(&_SendBuf[sPos], &sdata.Date[6], 2);
		sPos += 2;
		/* 区切り文字 */
		_SendBuf[sPos] = ',';
		++sPos;
		/* 店舗コード */
		memcpy(&_SendBuf[sPos], sdata.Shop, sizeof(sdata.Shop));
		sPos += sizeof(sdata.Shop);
		/* 区切り文字 */
		_SendBuf[sPos] = ',';
		++sPos;
		/* 店舗2コード */
		...
		++sPos;
		/* 上段バーコード */
		memcpy(&_SendBuf[sPos], sdata.Code1, sizeof(sdata.Code1));
		sPos += sizeof(sdata.Code1);
		/* 区切り文字 */
		_SendBuf[sPos] = ',';
		++sPos;
		/* 下段バーコード */
		...
		memcpy(&_SendBuf[sPos], "00000", 5);
		sPos += 5;
		memcpy(&_SendBuf[sPos], sdata.Num, sizeof(sdata.Num));
		sPos += sizeof(sdata.Num);
		/* 区切り文字 */
		_SendBuf[sPos] = ',';
		++sPos;
		/* 伝票番号 */
...
static short IDataSend(void)
{
	...
	for (i = 0; i < ctrl.IDataCnt; i++)
	{
		...
		memset(_SendBuf, ' ', sizeof(_SendBuf));
		/* 日付(YYYY/MM/DD) */
		memcpy(&_SendBuf[sPos], idata.Date, 4);
		sPos += 4;
		_SendBuf[sPos] = '/';
		++sPos;
		memcpy(&_SendBuf[sPos], &idata.Date[4], 2);
		sPos += 2;
		_SendBuf[sPos] = '/';
		++sPos;
		memcpy(&_SendBuf[sPos], &idata.Date[6], 2);
		sPos += 2;
		/* 区切り文字 */
		_SendBuf[sPos] = ',';
		++sPos;
		/* 店舗コード */
		memcpy(&_SendBuf[sPos], idata.Shop, sizeof(idata.Shop));
		sPos += sizeof(idata.Shop);
		/* 区切り文字 */
		_SendBuf[sPos] = ',';
		++sPos;
		/* 店舗2コード */
		memcpy(&_SendBuf[sPos], idata.Shop2, sizeof(idata.Shop2));
		sPos += sizeof(idata.Shop2);
		/* 区切り文字 */
		_SendBuf[sPos] = ',';
		++sPos;
		/* 上段バーコード */
		memcpy(&_SendBuf[sPos], idata.Code1, sizeof(idata.Code1));
		sPos += sizeof(idata.Code1);
		/* 区切り文字 */
		_SendBuf[sPos] = ',';
		++sPos;
		/* 下段バーコード */
		...
		memcpy(&_SendBuf[sPos], "00000", 5);
		sPos += 5;
		memcpy(&_SendBuf[sPos], idata.Num, sizeof(idata.Num));
		sPos += sizeof(idata.Num);
		_SendBuf[sPos] = ',';
		++sPos;
		/* 伝票番号 */
		memset(&_SendBuf[sPos], '0', 8);
...
static short URDataSend(void)
{
	...
	for (i = 0; i < ctrl.URDataCnt; i++)
	{
		...
		/* ----------------------------------------------------------------------------------------------  col01  :  日付(YYYY/MM/DD) 	*/
		memcpy(&_SendBuf[sPos], urdata.Date, 4);
		sPos += 4;
		_SendBuf[sPos] = '/';
		++sPos;
		memcpy(&_SendBuf[sPos], &urdata.Date[4], 2);
		sPos += 2;
		_SendBuf[sPos] = '/';
		++sPos;
		memcpy(&_SendBuf[sPos], &urdata.Date[6], 2);
		sPos += 2;
		_SendBuf[sPos] = ',';
		++sPos;

		/* ----------------------------------------------------------------------------------------------  col02  :  店舗コード		*/
		memcpy(&_SendBuf[sPos], urdata.Shop, sizeof(urdata.Shop));
		sPos += sizeof(urdata.Shop);
		_SendBuf[sPos] = ',';
		++sPos;

		/* ----------------------------------------------------------------------------------------------  col03  :  レジNo        */
		memcpy(&_SendBuf[sPos], ctrl.RejiNo, sizeof(ctrl.RejiNo));
		sPos += sizeof(ctrl.RejiNo);
		_SendBuf[sPos] = ',';
		++sPos;

		/* ----------------------------------------------------------------------------------------------  col03  :  担当コード		*/
		memcpy(&_SendBuf[sPos], urdata.Tanto, sizeof(urdata.Tanto));
		sPos += sizeof(urdata.Tanto);
		_SendBuf[sPos] = ',';
		++sPos;

		/* ----------------------------------------------------------------------------------------------  col04  :  レシートNo		*/
		memcpy(&_SendBuf[sPos], urdata.RecNo, sizeof(urdata.RecNo));
		sPos += sizeof(urdata.RecNo);
		_SendBuf[sPos] = ',';
		++sPos;

		/* ----------------------------------------------------------------------------------------------  col09  :  客数 		*/
		memcpy(&_SendBuf[sPos], urdata.KyakuNo, sizeof(urdata.KyakuNo));
		++sPos;
		_SendBuf[sPos] = ',';
		++sPos;

		/* ----------------------------------------------------------------------------------------------  col04  :  バーコード1 	*/
		memcpy(&_SendBuf[sPos], urdata.Code1, sizeof(urdata.Code1));
		sPos += sizeof(urdata.Code1);
		_SendBuf[sPos] = ',';
		++sPos;
		...
		memcpy(&_SendBuf[sPos], strBaika, sizeof(strBaika));
		sPos += sizeof(strBaika);
		_SendBuf[sPos] = ',';
		++sPos;
		...
		/* ----------------------------------------------------------------------------------------------  col07  :  数量 		*/
		memcpy(&_SendBuf[sPos], urdata.Num, sizeof(urdata.Num));
		sPos += 4;
		_SendBuf[sPos] = ',';
		++sPos;
		...
		memcpy(&_SendBuf[sPos], strBaika, 8);
		sPos += 8;
		_SendBuf[sPos] = ',';
		++sPos;

		/* ----------------------------------------------------------------------------------------------  col13  :  現金   	*/
		memcpy(&_SendBuf[sPos], urdata.Genkin, sizeof(urdata.Genkin));
		sPos += sizeof(urdata.Genkin);
		_SendBuf[sPos] = ',';
		++sPos;

		/* ----------------------------------------------------------------------------------------------  col14  :  実際は売掛	 */
		memcpy(&_SendBuf[sPos], urdata.Credit, sizeof(urdata.Credit));
		sPos += sizeof(urdata.Credit);
		_SendBuf[sPos] = ',';
		++sPos;
		...
		memcpy(&_SendBuf[sPos], strTaxRate, sizeof(strTaxRate));
		sPos += sizeof(strTaxRate);
		_SendBuf[sPos] = ',';
		++sPos;
		...
		/* ----------------------------------------------------------------------------------------------  col19 :  DateTime   */
		memcpy(&_SendBuf[sPos], urdata.Date, sizeof(urdata.Date));
		sPos += sizeof(urdata.Date);
		memcpy(&_SendBuf[sPos], urdata.Time, sizeof(urdata.Time));
		sPos += sizeof(urdata.Time);
...
static short NDataSend(void)
{
	...
	for (i = 0; i < ctrl.NDataCnt; i++)
	{
		...
		memset(_SendBuf, ' ', sizeof(_SendBuf));
		/* 日付(YYYY/MM/DD) */
		memcpy(&_SendBuf[sPos], ndata.Date, 4);
		sPos += 4;
		_SendBuf[sPos] = '/';
		++sPos;
		memcpy(&_SendBuf[sPos], &ndata.Date[4], 2);
		sPos += 2;
		_SendBuf[sPos] = '/';
		++sPos;
		memcpy(&_SendBuf[sPos], &ndata.Date[6], 2);
		sPos += 2;
		/* 区切り文字 */
		_SendBuf[sPos] = ',';
		++sPos;
		/* 店舗コード */
		memcpy(&_SendBuf[sPos], ndata.Shop, sizeof(ndata.Shop));
		sPos += sizeof(ndata.Shop);
		/* 区切り文字 */
		_SendBuf[sPos] = ',';
		++sPos;
		/* 店舗2コード */
		...
		++sPos;
		/* 上段バーコード */
		memcpy(&_SendBuf[sPos], ndata.Code1, sizeof(ndata.Code1));
		sPos += sizeof(ndata.Code1);
		/* 区切り文字 */
		_SendBuf[sPos] = ',';
		++sPos;
		/* 下段バーコード */
		...
		++sPos;
		/* 数量 */
		memcpy(&_SendBuf[sPos], ndata.Num, sizeof(ndata.Num));
		sPos += sizeof(ndata.Num);
		/* 改行コード */
		_SendBuf[sPos] = 0x0d;
		++sPos;
		_SendBuf[sPos] = 0x0a;
...
void AlfDataSend(short job)
{
	...
	{
	case JOB_SIIRE:
		_SndCnt = ctrl.SDataCnt;
		break;
	case JOB_TANA:
		_SndCnt = ctrl.TDataCnt;
		break;
	case JOB_IDO:
		_SndCnt = ctrl.IDataCnt;
		break;
	case JOB_URI:
		_SndCnt = ctrl.URDataCnt;
		break;
	}
TopFunc:
	offkey(1);
	...
	{
	case JOB_URI:
		_AlfInf.sendLen = 145 + 19 + 2 + 15;
		break;
	case JOB_SIIRE:
		_AlfInf.sendLen = 68 + 2 + 15 + 16;
		break;
	case JOB_TANA:
		_AlfInf.sendLen = 68 + 2 + 15 + 16;
		break;
