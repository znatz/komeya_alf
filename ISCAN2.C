...
static void Display( short item )
{
	switch( item ){
		case SHOP:
			...
			/* 店舗コード */
			if( idata.Shop[0] ){
				ckputsn( 9, 5, idata.Shop, sizeof( idata.Shop ), False, CLR_BASE );
				/*
				if( mmst.Code[0] ){
		...
		case SHOP2:
			...
			ckprintf( 10, 0, False, CLR_ID_TITLE, "%4d件", ctrl.IDataCnt );
			/* 店舗コード */
			//ckprintf( 9, 5, idata.Shop,  sizeof( idata.Shop  ), False, CLR_BASE );
			ckprintf( 9, 5, False,  CLR_BASE, "%s",idata.Shop  );
			//ckputsn( 2, 7,  mmst.Name, 		14, False, CLR_BASE );
			/* 店舗コード2 */
			if( idata.Shop2[0] ){
				ckputsn( 9, 10, idata.Shop2, sizeof( idata.Shop2 ), False, CLR_BASE );
				/*
				if( mmst2.Code[0] ){
		...
		case CODE1:
			if( SaveItem != CODE1 )
			{
				...
				ckputss( 0, 13, "F1:戻る F2:複数 ", False, CLR_BASE );
				ckputss( 0, 15, "        F4:削除 ", False, CLR_BASE );
		...
		case CODE2:
			...
			/* 下段コード */
			if( idata.Code2[0] ){
				//ckprintf( 11, 6, False, CLR_BASE, "\\%4d", atoln( &idata.Code2[8], 4 ) );
		...
		case DELETE:
			...
			{
				ClsColor();	/*   0123456789012345*/
				ckputss( 0,  0, "<削除>          ", False, CLR_TITLE );
			...
			/* 上段コード */
			if( idata.Code1[0] ){
				ckputsn( 3, 3, idata.Code1, 13, False, CLR_BASE );
			}
			/* 下段コード */
			if( idata.Code2[0] ){
				ckputsn( 3, 6, idata.Code2, 13, False, CLR_BASE );
			}
			/* 数量 */
			if( !idata.Num[0] ){
				memcpy( idata.Num, "00001", sizeof( idata.Num ) );
...
void Iscan( void )
{
	...
	SaveItem = -1;
	while( item ){
		if( MaxCheck( ctrl.IDataCnt, IDATA_MAX ) ) return;
		Display( item );
		switch( item ){
			case SHOP:
				ret = NumInput( 9, 5, idata.Shop, sizeof( idata.Shop ), 0, 9L, 
								KEY_FIX | IN_NUMERIC | KEY_FUNC, TYPE_CHAR, NO_CHECK );
				break;
			case SHOP2:
				ret = NumInput( 9, 10, idata.Shop2, sizeof( idata.Shop2 ), 0, 9L, 
								KEY_FIX | IN_NUMERIC | KEY_FUNC, TYPE_CHAR, NO_CHECK );
				break;
			//case TANTO:
			...
			case CODE1:
				...
				break;
			case CODE2:
				ret = CodeInput( 3, 8, idata.Code2, sizeof( idata.Code2 ), 
								 BCR_NOTDRW | BCR_WPC | KEY_FUNC );
				break;
			case NUM:
		...
		if( ret == ENTRY || ret == SENTRY || ret == SKIP || ret == BARENT ){
			...
			if( item == SHOP2 ){
				// 店舗マスタ検索 
				int intChk = strcmp(idata.Shop[0],idata.Shop2[0]);
				if( intChk == 0 ){
					item=SHOP;
					memset( idata.Shop2, 0x00, sizeof( idata.Shop2 ) );
					beep( 50, 1 );
					continue;
				}
			}
			...
			/*else if( item == TANTO ){
				...
				if( !memcmp( idata.Code1, "7", 1 ) ){
					item = CODE2;
					continue;
				/* 上段コードの先頭から1桁が9の場合、下段コード入力へ */
				} else if( !memcmp( idata.Code1, "9", 1 ) ){
					item = CODE2;
					continue;
				/* 上段コードの先頭から2桁が21の場合、下段コード入力へ */
				} else if( !memcmp( idata.Code1, "21", 2 ) ){
					item = CODE2;
					continue;
				} else if( !memcmp( idata.Code1, "45", 2 ) || !memcmp( idata.Code1, "49", 2 ) ||
						   !memcmp( idata.Code1, "20", 2 ) ){
					...
					continue;
				} else if( !memcmp( idata.Code1, "2289", 4 ) || !memcmp( idata.Code1, "2299", 4 ) ){
					item = CODE2;
					continue;
			...
			if( item == CODE1 ){
				...
				if( Func == 1 ){
					Func = 2;
					ckputss( 0, 11, "数量:           ", False, CLR_BASE );
					ckputss( 0, 13, "F1:ﾒﾆｭｰ F2:1点  ", False, CLR_BASE );
				}
				else if( Func == 2){
					Func = 1;
					ckputss( 0, 11, "                ", False, CLR_BASE );
					ckputss( 0, 13, "F1:ﾒﾆｭｰ F2:複数 ", False, CLR_BASE );
				}
			}
			continue;
		} else if( ret == F1KEY ){
			if( item == CODE1){
				memset( idata.Shop, 0x00, sizeof( idata.Shop ) );
				memset( idata.Shop2, 0x00, sizeof( idata.Shop2 ) );
				item = SHOP;
				continue;
			}
			else if( item == CODE2 || item == NUM ){
				meisaiclr();
				if( item == NUM ){
					Func = 1;
					item = CODE1;
