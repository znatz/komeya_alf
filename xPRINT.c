...
int LapinBTOpen(char* btAddr, bool CRC_Check, int commandMode)
{
	...
	for (i = 0; i < 3; i++) {
		...
		ret = ropen( PORT_BLUETOOTH );
		/*
		 * BtConnect((unsigned char*)bt_addr, UUID_SPP);

		 * 12 SSP認証 パスコード入力要求 パスキー入力を促しBtSSPPin関数で認証
		 * 11 SSP認証 パスコード表示要求 BtGetSSPPin関数でパスキーを取得し表示する
		 * 10 SSP認証 パスコード許可要求 認証可否を選択しBtSSPConf関数で認証
		 * 0 正常終了（接続成功）
		 * -1 パラメータエラー
		 * -4 システムエラー
		 * -5 タイムアウト
